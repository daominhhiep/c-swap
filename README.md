...
Multi-account switcher for Claude Code — and, in this fork, for the OpenAI Codex CLI. Easily switch between multiple Claude accounts without logging out, or let it switch for you before you hit a rate limit. Track usage for every account in a live dashboard, and run accounts in parallel. Works with both the Claude Code CLI and the VS Code extension.

`ccswap` is a fork of [claude-swap](https://github.com/realiti4/claude-swap). It installs as a separate command so it can live next to the upstream `cswap`, and it shares the same data directory, so every Claude account you already added with `cswap` shows up in `ccswap` right away (don't run both tools' auto-switchers at the same time).
...
```bash
uv tool install git+https://github.com/daominhhiep/c-swap
```

### Using pipx

```bash
pipx install git+https://github.com/daominhhiep/c-swap
```

### From source

```bash
git clone https://github.com/daominhhiep/c-swap.git
cd c-swap
uv sync
uv run ccswap help
```
...
Run `ccswap` on its own (or `ccswap tui`) for the full-screen dashboard: live usage for every account, switching, and the auto-switcher, all keyboard-driven. `ccswap watch` opens it straight to the live monitor. Works on macOS, Linux, and Windows.
...
```bash
uv tool install 'ccswap[menubar] @ git+https://github.com/daominhhiep/c-swap'   # or: pipx install 'ccswap[menubar] @ git+https://github.com/daominhhiep/c-swap'
ccswap menubar
```
